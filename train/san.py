...
# ------------------------------------ Model Config -------------------------------------------------------------------------------------------
    ...
    model_dim=60,
    sentence_representation_dim=60,
...
def dot_product_attention_relative(q, k, v, name=None):
    ...
    logits = logits / (dim_size**(0.5))     # Scale the dot product # 开根号
...
def multi_head_attention(input_sequence, dropout_keep_prob_tensor):
    ...
    print("input_sequence {}".format(input_sequence))
    ...
    print("san_output: {}".format(san_output))
...
def createAndTrainTransformer(datasets, wordIndxToVecMatrix, wordIndxToVec_tensor, output_dim, datasetName, max_sentence_length):
    ...
    x_tensor = tf.compat.v1.placeholder(tf.int32, [None, max_sentence_length], name="input_sentence")     # [batch_size, num_steps, input_dim]
    y_tensor = tf.compat.v1.placeholder(tf.float32, [None, output_dim], name="targets")                  # [batch_size, target_dim] We're predicting only 1 number at the end...
    ...
    with tf.Session(config=config) as sess:
        ...
        strTimeStr = str(datetime.date(start_time))
        ...
        print("wordIndxToVecMatrix shape: {}".format(wordIndxToVecMatrix.shape))
        # printNumberOfParams()
        print("self_attention_heads {}".format(hp.self_attention_heads))
        for iteration in range(1, hp.num_training_iterations):
            ...
            if iteration % hp.run_on_val_and_test_every == 0:
                ...
                if acc_val >= best_acc_val:
                    ...
                    print("---------------------------------------------------------------------------------------------------------------")
                    print("\n")
                    # modelSaveName = base_model_dir + strTimeStr + "-(data={})-(iteration={})-(val_acc={:.4f})-(test_acc={:.4f}).ckpt".format(datasetName, iteration, acc_val, acc_test)
                    modelSaveName = base_model_dir + strTimeStr
...
def run_model_on_datasets_with_embeddings(embedding_file, file_type):
    ...
    for datasetName in datasetNames:
        ...
        # personal dataset
            ...
            dataset = General_Dataset('datasets/yelp',
                                             None,
                                             one_hot=True,
                                             rep=words)
        ...
        # Store word_idx_map
        with open('model/word_idx_map.json', 'w') as f:
        ...
        for i in range(1, hp.run_exps_amount + 1):  
            tf.compat.v1.reset_default_graph()  # Clears the current loaded tensorflow graph.

            w2i = tf.Variable(tf.constant(0.0, shape=[W.shape[0], W.shape[1]]),
                trainable=False, name="embedding_table")
            wordIndxToVec_tensor = tf.compat.v1.placeholder(tf.float32, [W.shape[0], W.shape[1]], name="embedding_table")     # [vobab_size x word_embedding_dim]
